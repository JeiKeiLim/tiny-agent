...
from kestrel.data.sft_chat import IM_ASSISTANT, render_sft
...
def build_chat_prompt(messages: list[dict[str, Any]], tokenizer: Tokenizer) -> str:
    """Render a system/user/assistant chat history with the SFT chat template."""
    row = SFTRow.model_validate({"source": "chat", "messages": messages})
    return render_sft(row, tokenizer).text
